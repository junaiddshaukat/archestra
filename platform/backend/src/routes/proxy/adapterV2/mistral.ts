 ...
 * This adapter reuses OpenAI's logic with Mistral-specific configuration.
 *
  ...
  implements LLMRequestAdapter<MistralRequest, MistralMessages>
{
  ...
  convertToolResultContent(messages: MistralMessages): MistralMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to Mistral format
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[MistralAdapter] Stripped browser tool results from messages",
        );
      }
    }
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[MistralAdapter] Stripped image blocks from tool result",
    );
  }
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for Mistral
...
> = {
  ...
  createClient(
    ...
    // Use OpenAI SDK with Mistral base URL (OpenAI-compatible API)
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (same for Mistral)
